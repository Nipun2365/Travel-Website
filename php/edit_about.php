...
$about = []; //........ Initialize an empty array to store the about data........//

if(isset($_GET['pid'])) {
    $about_pid = $_GET['pid'];
    $about_query = "SELECT * FROM about WHERE id='$about_pid'";
    $result = mysqli_query($connection, $about_query);
    ...
    if ($result) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo "No about data found with ID: $about_pid";
        }
    } else {
...
if(isset($_POST['send']))
{
    ...
    $query ="UPDATE about SET description='$description', image='$image' WHERE id=$about_pid";

    $result = mysqli_query($connection, $query);

    if($result){
        $_SESSION['message'] = "update success";
        header('location:ab_dashboard.php');
        exit(0);
    }
}
    ...
    <section class="form-container">
        <form action="" method="post">
            ...
            <div class="ad_package">
                ...
                <input type="file" name="image" required value="<?= $about['image'] ?? ''; ?>"><br>
